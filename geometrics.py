...
def set_id_grid(depth):
    ...
    i_range = Variable(torch.arange(0, h).view(
        1, h, 1).expand(1, h, w)).type_as(depth)  # [1, H, W]
    j_range = Variable(torch.arange(0, w).view(
        1, 1, w).expand(1, h, w)).type_as(depth)  # [1, H, W]
...
def pixel2cam(depth, intrinsics_inv):
    ...
    current_pixel_coords = pixel_coords[:, :, :h, :w].expand(
        b, 3, h, w).contiguous().view(b, 3, -1)  # [B, 3, H*W]
...
def flow_warp(img, flow, padding_mode='zeros'):
    ...
    grid_x = Variable(torch.arange(0, w).view(1, 1, w).expand(
        1, h, w), requires_grad=False).type_as(u).expand_as(u)  # [bs, H, W]
...
# rigid flow from depth and pose
def pose2flow(depth, pose, intrinsics, intrinsics_inv, rotation_mode='euler', padding_mode=None):
    ...
    grid_x = Variable(torch.arange(0, w).view(1, 1, w).expand(
        1, h, w), requires_grad=False).type_as(depth).expand_as(depth)  # [bs, H, W]
    grid_y = Variable(torch.arange(0, h).view(1, h, 1).expand(
        1, h, w), requires_grad=False).type_as(depth).expand_as(depth)  # [bs, H, W]
...
def mask_gen(flow, rigid_flow):
    ...
    dist=torch.norm(flow-rigid_flow,dim=1) # [B,2,H,W]->[B,H,W]
    mask=torch.exp(-dist)
    return mask
...
# forward and backward depth generation


# forward and backward flow generation
