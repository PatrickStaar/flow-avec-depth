...
def train(net, dataloader, device, optimizer, cfg):
    net.train()
    ...
    for i, input_dict in process:
        ...
        for t in range(T-1):
            ...
            p1=poses[t+1]
            d0, d1=depth_maps[t], depth_maps[t+1]

            d0 = [upsample(d,(H,W)) for d in d0]
            d1 = [upsample(d,(H,W)) for d in d1]
            
            warped_forward = [
                inverse_warp(img0, d.squeeze_(1),p1,intrinsics,intrinsics_inv) \
                    for d in d1
            ]

            # warped_backward = [
            #     inverse_warp(
            #         img1, d.squeeze_(1),-p1,intrinsics,intrinsics_inv) \
            #             for d in d0
            # ]
            ...
            for k in loss_per_iter.keys():
                loss_per_iter[k]+=loss_forward.get(k,0)
            ...
            # for k in loss_per_iter.keys():
            #     loss_per_iter[k]+=loss_backward.get(k,0)
    ...
    for k, v in loss_per_epoch.items():
        loss_per_epoch[k] = v/len(dataloader)/(seq_len-1)
...
def eval(net, dataloader, device, cfg):
    ...
    with torch.no_grad():
        for i, input_dict in val_process:
            ...
            depth_maps, _= net(seq)
            seq_len=len(depth_maps)
    ...
    for k, v in loss_per_validation.items():
        loss_per_validation[k] = v/len(dataloader)/seq_len
